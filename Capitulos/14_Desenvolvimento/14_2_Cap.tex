...
\begin{figure}[H]
    ...
    \centering
    \includegraphics[scale = .35]{Imagens/Desenvolvimento/Cap2/diagrama_servidores.png}
    \centering
    \vspace{0.2cm} \\
    Fonte: Autor.
...
\par Os níveis de operação clássicos do SCADA mapeiam-se naturalmente sobre a arquitetura. O Nível 0, dos equipamentos de campo e seus sensores, corresponde aos transmissores, medidores e demais dispositivos físicos instalados nas unidades operacionais dos clientes. O Nível 1, da supervisão local junto aos IEDs, é representado pelos CLPs e modems no campo, que executam a comunicação Modbus com os instrumentos e, quando há, oferecem uma IHM simples para operação no próprio ponto. O Nível 2, da supervisão da instalação como um todo, é materializado pelos workers de aquisição, pelo Moleculer e pelos serviços de domínio na nuvem (cadastro, alarmes, históricos), que agregam os pontos relevantes de cada cliente em estações redundantes com telas elaboradas para supervisão completa da unidade. O Nível 3, do centro de operação remota, é o frontend Next.js acessado via navegador, que consolida pontos provenientes de múltiplas instalações geograficamente distribuídas, disponibilizando apenas o que é estritamente necessário para a gestão sistêmica.
...
\begin{figure}[H]
    ...
    \caption{Gateway ABS}
    \centering
    \includegraphics[scale = .4]{Imagens/Desenvolvimento/Cap2/gateway_abs.jpeg}
    \centering
    \vspace{0.2cm} \\
    Fonte: Autor.
